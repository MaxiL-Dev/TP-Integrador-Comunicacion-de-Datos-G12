...
export const IconButton = ({ iconName, variant = 'default', isActive = false, className = '', ...props }: IconButtonProps) => {
  ...
  const variants = {
    // Agregamos 'group' al inicio de esta cadena
    record: "group w-16 h-16 rounded-full bg-[#a8c7fa] text-[#6497cd] hover:scale-105 cursor-pointer hover:bg-[#2c344d]",
  ...
  return (
    <button className={`${baseStyles} ${variants[variant]} ${className}`} {...props}>
      {variant === 'record' ? (
        // Agregamos group-hover:bg-[#1a2030] para que se oscurezca al pasar el mouse
        <div className={`transition-all duration-200 ease-in-out bg-[#6497cd] group-hover:bg-[#1a2030] ${isActive ? 'w-5 h-5 rounded-sm' : 'w-6 h-6 rounded-full'}`} />
      ) : (
